...
You can see clear improvements in the inference results after optimizations to reach .90 mask mAP (top right) and the model can even predict cards that are occluded (bottom right). Early iterations of the model were trained on a dataset of only 5,000 images having 1 card each, limiting its versatility. Later versions were trained on up to 50,000 images, each having up to 5 cards with more randomness and even considerable overlap. A larger dataset and fewer epochs helped to greatly improve the model's precision. I also omitted common and uncommon cards to steer the dataset towards more rare types like full art cards that are more likely to be scanned by users and also don't have a simple yellow border.
...
## Q: Why not use deep learning for the whole process including identification?
